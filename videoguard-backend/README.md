...
[videoguard-backend]  Spring Boot 4 / Java 17 / JPA
   │                    └─ OpenAI Chat API (발언 리스크·자막 비교 판정)
   │  HTTP
[videoguard-analysis] FastAPI / Python
...
cd ../videoguard-analysis && .\run.ps1
...
`application.yml`의 `videoguard.analysis.enabled-analyzers`에서 켜고 끈다.
...
절대경로 대신 `storageKey`를 쓴다. 로컬에서는 `videoguard.storage.location` 기준 상대 경로다.
...
분석 서버 없이 파이프라인만 보려면 `videoguard.use-dummy-transcript: true`.
