...
/**
 * [기본 비활성] 화면 자막과 발언을 대조해 자막이 의미를 바꿨는지 본다.
 *
 * application.yml 의 enabled-analyzers 에서 빠져 있어 실행되지 않는다.
 *
 * 끈 이유:
 * 자막이 발언과 다른 것은 원래 정상이다. 예능 자막, 요약 자막, 효과음 표기는
 * 발언을 그대로 옮기지 않는다. 게다가 OCR 이 한글을 자주 틀리게 읽어서
 * ("부모를 놀라게 울우 아이의 종이로가려진") 그 오인식까지 왜곡으로 판정됐다.
 * 글자 일치율 필터를 넣어도 어순이 뒤섞이면 걸러지지 않아 오탐이 대부분이었다.
 *
 * 지금은 발언과 화면 자막을 각각 독립적으로 분석하고,
 * 같은 시간대에 같은 유형이 잡히면 FindingFusionService 가 한 건으로 합친다.
 *
 * OCR 정확도가 충분히 올라가면 다시 켜볼 만하다.
 * 자막으로 발언을 자극적으로 바꾸는 어그로성 편집은 실제로 논란거리이기 때문이다.
 *
 * ---
 *
