...
public class SpeechReviewAnalyzer implements ContentAnalyzer {
     ...
     *
     * 관문은 하나같이 "이름을 댈 수 있는가" 를 묻는데, 모델은 못 대면 빈칸으로
     * 두는 대신 '특정 인물' 같은 분류명을 적어 넣는다. 형식은 채워졌지만
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            **관문: 평가받은 대상의 이름을 댈 수 있는가?**
            "OO식당 김치찌개", "OO 브랜드 버거", "부산 어묵", "그 가게 사장님"
            처럼 **고유명사나 지역명이 붙은 대상**을 적을 수 있어야 한다.
            '특정 인물', '어떤 가게' 는 이름이 아니다. 그러면 빼라.

            걸리는 예:
              "너무 특색이 없어가지고"          가게 메뉴에 대한 평가
    ...
    private String buildPrompt(List<TranscriptSegment> window) {
        ...
        return "다음은 영상 대본이다.\n"
                ...
                + "실명 대상 평가, 속성을 근거로 한 차별 다섯 중 하나에\n"
