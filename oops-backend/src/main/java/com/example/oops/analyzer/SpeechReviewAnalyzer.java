...
public class SpeechReviewAnalyzer implements ContentAnalyzer {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            **제작자가 다시 볼 만한 지점과 그 이유를 알려주는 것**이다.
            ...
            아래 셋은 **다른 유형보다 먼저, 더 꼼꼼히** 봐라.
            제작자가 스스로 알아채기 가장 어려운 것들이라 이 도구의 값어치가 여기서 나온다.
            해당하면 점수를 0.6 이상으로 주고, 놓치는 것보다 낮은 점수로라도 올려라.

            (가) 커뮤니티 맥락  → UNFAMILIAR_CONTEXT
                 특정 커뮤니티·밈·은어가 일반적인 뜻과 다르게 쓰이는 경우.
                 어미와 말버릇도 포함한다.

            (나) 집단 일반화    → GENERALIZATION
                 성별·세대·지역·직업·국적 등 집단 전체를 하나로 묶거나 낮춰 말하는 경우.

                 세 가지가 함께 있으면 걸린다.
                   · 집단을 가리키는 말   (여자, 20대, 경상도 사람, 공무원)
                   · 전체로 묶는 표현     (다, 원래, 역시, 죄다, 믿고 걸러)
                   · 그 집단에 붙인 성질  (감정적이다, 책임감이 없다)

                 걸리는 예: "요즘 20대는 다 책임감이 없어", "역시 OO 사람들은"
                 안 걸리는 예: "제 친구는 책임감이 없어요"  ← 개인
                              "20대 투표율이 낮습니다"     ← 사실 서술

            (다) 정치 맥락     → UNFAMILIAR_CONTEXT
                 정치인·정당·지지층·이념을 향한 정치 은어나 낙인 표현.
                 **좌우를 가리지 말고 같은 기준을 적용해라.** 한쪽만 잡으면 그게 편향이다.
                 지지 표명이나 정책 비판 자체는 대상이 아니다.

            나머지 유형(비하·조롱·민감 주제 등)도 계속 보되,
            위 셋이 함께 걸리면 위 셋을 우선해서 적어라.
            ...
            **사실 관계는 네가 볼 몫이 아니다.**
            ...
            - GENERALIZATION: 집단 전체를 단정하는 대목. **집중해서 볼 유형 (나)**
              성별·세대·지역·직업·국적을 하나의 성질로 묶는 말이다.
