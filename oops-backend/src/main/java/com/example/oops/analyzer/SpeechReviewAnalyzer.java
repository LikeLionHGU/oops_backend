...
public class SpeechReviewAnalyzer implements ContentAnalyzer {
     ...
     * 20줄은 대략 40초 분량이고, 그 안에 정말로 다시 볼 지점이 넷 이상 있는 일은 드물다.
     * 모델이 넷 이상 보내면 점수 높은 셋만 남긴다.
     *
     * **2 였다가 3 으로 올렸다.** 대상 평가와 차별을 되살리면서 한 창에
     * 정당한 지적이 셋 나오는 경우가 생겼다. 1.5분 영상에서 롯데리아 평가와
     * 지역 음식 평가가 같은 창에 있었는데 둘 중 하나가 잘렸다.
     * 오탐 134건을 막은 것은 이 상한이 아니라 각 유형의 관문이다.
     * 상한은 "모델이 폭주했다" 를 알아채는 안전장치로만 쓴다.
     */
    ...
    /**
     * 대상 이름이 아니라 '대상의 종류' 를 적은 것. 버린다.
     *
     * 관문은 "무엇을 두고 하는 말인지 적을 수 있는가" 를 묻는데, 모델은 못 적으면
     * 빈칸으로 두는 대신 '특정 인물' 같은 분류명을 적어 넣는다. 형식은 채워졌지만
     * 제작자는 누구를 말하는지 알 수 없어서 열어봐도 확인할 것이 없다.
     * 실제로 '특정 인물' 카드가 올라왔다.
     *
     * 목록으로만 막으면 '특정인', '어떤 인물', '특정 브랜드' 로 새어 나온다.
     * 그래서 접두어로 본다 — '특정' · '어떤' 으로 시작하는 대상은 이름이 아니다.
     *
     * **여기에 '메뉴', '가게', '브랜드', '지역' 을 넣지 마라.**
     * 그건 무엇이 평가받았는지는 알려주므로 카드로서 값이 있고,
     * 정작 사용자가 원하는 메뉴 비하·브랜드 비하를 같이 죽인다.
     */
    ...
    /**
     * 홀로 쓰이면 대상이 아니라 분류명인 말.
     *
     * '중국인' 이나 '20대' 처럼 실재하는 집단은 넣지 마라. 그건 진짜 대상이다.
     */
    static final java.util.Set<String> PLACEHOLDER_WORDS = java.util.Set.of(
            "인물", "누군가", "타인", "제3자", "제삼자", "당사자",
            "대상", "상대", "상대방", "출연자", "게스트", "화자");
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            **제작자가 다시 볼 만한 지점과 그 이유를 알려주는 것**이다.
            ...
            지나가듯 흘린 한마디는 말한 본인이 기억조차 못 한다.

            **다섯 모두 관문이 있다.** 관문은 하나같이
            "무엇을 두고 하는 말인지 적을 수 있는가" 를 묻는다.
            적을 수 없는 항목은 제작자가 열어봐도 확인할 것이 없다. 그러면 빼라.
            **다만 '적을 수 있다' 가 곧 '고유명사가 있다' 는 아니다.**
            ④ 는 영상에서 지금 다루는 대상이면 이름이 없어도 통과한다.
            ...
            **무엇인지 특정되는 실재하는 대상**을 깎아내리는 대목.
            가게, 메뉴, 음식의 맛, 브랜드·체인, 제품, 작품, 지역 음식·특산물,
            사람이 대상이다.

            **영상이 그 대상을 소개·방문·시식하는 중이어야 하는 게 아니다.**
            대화 중에 그냥 언급하며 평가한 것도 해당한다. 토크·인터뷰 채널에서는
            이쪽이 훨씬 흔하다. 앉아서 이야기만 하다가 특정 체인이나 지역 음식을
            깎아내리는 대목이 나온다.

            말한 본인은 지나가듯 흘린 말이라 기억조차 못 하는 경우가 많은데,
            당사자는 그 장면만 잘라서 본다. 그래서 제작자가 놓치는 자리다.

            **관문: 평가받은 대상이 무엇인지 적을 수 있는가?**

            둘 중 **하나만** 되면 통과다.

              (1) 고유명사·지역명이 붙은 대상
                  "OO식당 김치찌개", "OO 버거", "부산 어묵"
              (2) 영상에서 지금 다루고 있는 실물
                  "이 가게 메뉴", "지금 먹는 음식", "사장님"
                  **이름이 안 나와도 된다.** 가게 이름을 한 번도 말하지 않는
                  영상이 대부분이고, 그래도 당사자는 자기 가게인 걸 안다.

            **(2) 를 빼먹지 마라.** 관문을 고유명사로만 두면 이름을 말하지 않는
            음식점 영상에서 평가 대목이 통째로 빠진다. 실제로 그랬다.

            통과가 아닌 것은 **분류명만 적은 경우**다 —
            '특정 인물', '어떤 브랜드' 는 무엇인지 적은 것이 아니다.

            걸리는 예:
              "너무 특색이 없어가지고"          이 가게 메뉴에 대한 평가
                                              (가게 이름이 안 나와도 올린다.
                                               target 은 "메뉴" 나 "가게" 로 적으면 된다)
              "할머니 살을 뜯는 것 같은 맛"     먹고 있는 음식에 대한 평가
              "OO 버거는 진짜 못 먹겠다"        체인 브랜드의 음식에 대한 평가
              "OO 지역 음식은 다 짜고 별로"     지역 음식에 대한 평가
              "돈이 아깝다", "여기 왜 오지"     가게에 대한 평가

            **올리지 마라. 아래는 전부 실제로 잘못 잡았던 것들이다.**
              화자 자신에 대한 말   "나 못해", "공부나 해야겠다", "내가 바보지"
              화자의 소속·작품     자기 채널·자기 영상에 대한 자평
              대상이 대명사뿐      "저거 좀 아니다"  ← 무엇인지 적을 수 없다
              대상이 분류명뿐      '특정 인물', '어떤 브랜드'  ← 이름이 아니다
              칭찬·사실 서술       "맛있다", "양이 많네요", "가격은 8000원입니다"

            **완곡하게 말한 것도 평가다.**
            한국어에서 부정 평가는 대개 돌려 말한다.
            "특색이 없다", "무난하다", "그냥 그렇다", "특별한 건 없네",
            "굳이 올 필요는" 은 중립 서술이 아니라 낮춰 말한 것이다.
            세게 말하지 않았다고 넘기지 마라. 당사자는 그렇게 읽지 않는다.

            **상호명이 관용구로 쓰인 경우만 빼라.**
            "OO 같은 소리 하고 있어" 는 상호를 빌린 말버릇이고 그 브랜드 평가가 아니다.
            **하지만 상호가 나왔다고 무조건 관용구로 넘기지 마라.**
            그 브랜드의 음식·가격·서비스·매장을 실제로 평가했으면 올려라.
            판단 기준은 하나다 — **브랜드 이름을 다른 말로 바꿔도 문장이 성립하면
            관용구, 그 브랜드여야 뜻이 통하면 평가다.**
            ...
            **국적과 지역이 가장 자주 나오고 가장 자주 놓친다.**
            국적이나 지역을 품질·위생·성향의 근거로 삼는 말은 대화에서 워낙
            자연스럽게 나와서 화자도 편집자도 걸러내지 못한다.
            "중국산이니까", "OO 지역이라 그렇지" 가 그렇다.
            **이건 사실 서술이 아니라 판단의 근거로 쓴 것이다.**

            **관문: 어떤 속성을 어떤 판단의 근거로 삼았는지 적을 수 있는가?**
            ...
            걸리는 예:
              ...
              "중국 거는 못 믿어"         국적을 신뢰 판단의 근거로 삼았다
              "OO 지역이라 그런가 보다"   지역을 성향 판단의 근거로 삼았다
              "장애인 같이 왜 그래"       장애를 비하 비유로 썼다

            **올리지 마라.**
              속성 언급 자체   "중국에서 만든 제품입니다", "여성 참가자가 많네요"
              사실 서술       "이 공장은 중국에 있습니다", "중국산이 더 쌉니다"
              집단 전체를 묶은 말  그건 ② 다. 아래를 봐라.

            **언급과 근거 삼기를 가르는 기준.**
            그 속성을 빼면 판단이 무너지면 ⑤ 다.
            "중국산이니까 그렇지" 에서 국적을 빼면 아무 말도 안 남는다.
            "중국에서 만들었습니다" 는 국적을 빼면 그냥 정보가 하나 줄어든다.
            ...
            2. 대상이 없거나, 화자 자신이거나, 관용 표현이면 넘어간다.
               ...
               **반대로, 관용구처럼 들린다고 넘기지도 마라.**
               상호를 다른 말로 바꿔도 문장이 성립하면 관용구,
               그 상호여야 뜻이 통하면 그 브랜드에 대한 평가다. 후자는 ④ 다.
            ...
            - 대상이 특정되지 않는 일반적인 감상
            - 일상 대화, 잡담, 근황 이야기
              (**이름이 붙은 대상을 평가했으면 잡담이 아니라 ④ 다.**
               음식 이야기라서, 앉아서 떠드는 중이라서 넘기지 마라.
               "요즘 뭐 먹었어" 는 잡담이고
               "OO 버거 진짜 못 먹겠더라" 는 ④ 다.)
            ...
            - "특정 체인의 음식을 평가하는 대목입니다. 브랜드 당사자가 볼 수 있습니다."
            - "특정 지역의 음식을 평가하는 대목입니다."
    ...
    private List<RiskFinding> analyzeWindow(AnalysisContext context,
        ...
        for (LlmFinding item : result.findings()) {
            ...
            // 대명사·지시어, 그리고 '특정 인물' 같은 분류명은 대상이 아니다.
            if (isPlaceholderTarget(item.target())) {
                log.debug("[speech-risk] 대상이 이름이 아니라 분류명이라 버립니다 — '{}' / {}",
    ...
    private String buildPrompt(List<TranscriptSegment> window) {
        ...
        return "다음은 영상 대본이다.\n"
                ...
                + "대상 평가, 속성을 근거로 한 차별 다섯 중 하나에\n"
                + "확실히 해당하는 줄만 찾아라. 없으면 빈 배열로 답한다.\n"
                + "대상은 고유명사나 지역명으로 적어라. '특정 인물' 은 이름이 아니다.\n\n"
                + lines;
    }
